...
require_once __DIR__ . '/../../includes/TranslationActivity.php';*/
	...
	if (in_array($filter, $keys)) {
		if ($filter == 11) {
			...
			if ($to) {
				...
				LEFT JOIN users as t8
				ON t8.user_id = t1.user_id
				WHERE t1.to_language_id = '$filter'
				ORDER BY t1.date DESC";
			}
			else {
				$sql = "SELECT t1.from_function_id, t5.language_name as from_language_name, t2.language as from_language_id, t2.function_name, t2.description, t1.to_language_id, t3.language_name as to_language, COALESCE(t4.answers, 0) as answers, t1.upvotes as upvotes, t6.feedback_id as feedback, t1.views, t8.user_id, t8.username, t8.points, t8.avatar, t1.linked_id, t1.action_type as `identifier`, TIME_TO_SEC(TIMEDIFF(NOW(), t1.last_updated)) as `action_date`
				FROM translations as t1
				INNER JOIN functions as t2
				ON t2.function_id = t1.from_function_id
